...
export const DynamicBoundariesNoSsr = dynamic(
  () => import('../../features/poc/Boundaries/Boundaries'),
  {
    ssr: false,
  }
)
...
async function fetchInstancesPerClass() {
  ...
  return instancesPerClass
    .map(item => ({
      ...item,
      parent:
        parents[Math.floor((Math.random() * instancesPerClass.length) / 10) + 1] ?? parents[0],
    }))
    .slice(0, 1000)
}
...
const Boundaries = () => {
  const { isLoading, isError, data } = useQuery(['instances-per-class'], () =>
