...
} from '../GalaxyInterface/ZoomStates'
...
export function useRouteTransitions() {
  ...
  useEffect(() => {
    function init() {
      ...
      const isTheSamePage =
        params?.page && router.query.page && (params?.page as string) === router.query.page
      const isTheSameZoom =
      ...
      if (zoomStatesZoom5Galaxy.includes(activeZoom)) {
        const navigatedFromZoom5Page = prevActiveZoom
          ? zoomStatesZoom5Galaxy.includes(prevActiveZoom)
          : false
        if (!params) {
          console.error('params are needed for these states')
          return
