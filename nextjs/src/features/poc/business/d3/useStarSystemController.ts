...
export function useStarSystemController(
...
function ticked(
  ...
  node
    .attr('cx', (d: D3CollectionItem) => d.x ?? 0)
  ...
  nodeForeign
    .attr(
      'x',
      (d: D3CollectionItem) => (d.x ?? 0) - getDiameterFromDataDimensions(dataDimensions, d)
    )
    .attr(
      'y',
      (d: D3CollectionItem) => (d.y ?? 0) - getDiameterFromDataDimensions(dataDimensions, d)
    )
