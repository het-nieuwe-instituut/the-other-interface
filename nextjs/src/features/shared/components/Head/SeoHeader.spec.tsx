...
describe('SEO header', () => {
    it('it should be able to render the seo header', () => {
        const renderComponent = () =>
            render(<SeoHeader title="test" preventIndexing={true} imageUrl="test" fullUrl="fullurl.com" />, {
        ...
        expect(document.querySelector("meta[property='og:image']")?.attributes.getNamedItem('content')?.value).toBe(
            'test'
        )
        expect(document.querySelector("meta[name='robots']")?.attributes.getNamedItem('content')?.value).toBe('noindex')
    })
