...
export const ButtonsGrid: React.FC<Props> = props => {
  ...
  function renderButtons() {
    return props.buttons?.map((button, index, array) => {
      const config =
        buttonConfig[props.buttonStyle ?? EnumComponentmodulesbuttonsmoduleButtonstyle.Default]
  ...
  return (
    <NextLink style={{ width: '100%' }} href={getURl(button)} passHref>
      <Button
        ...
        target={
          !!button.url && isExternalURL(button.url, process.env.parsed.NEXT_PUBLIC_URL ?? '')
            ? '_blank'
            : undefined
        }
